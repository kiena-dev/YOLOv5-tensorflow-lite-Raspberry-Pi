...
# Fungsi untuk menampilkan dalam LCD
class lcd:
    def __init__(self):
        # Initialize lcd
...
def control_led(cls):
    ...
    if cls == 1: # Jika kelas mobil
        GPIO.output(red_pin, GPIO.HIGH)
        time.sleep(1)
        GPIO.output(red_pin, GPIO.LOW)
    elif cls == 3: # Jika kelas orang
        GPIO.output(green_pin, GPIO.HIGH)
        time.sleep(1)
        GPIO.output(green_pin, GPIO.LOW)
    elif cls == 4: # Jika kelas truk
        GPIO.output(yellow_pin, GPIO.HIGH)
        time.sleep(1)
        GPIO.output(yellow_pin, GPIO.LOW)
    elif cls == 0: # Jika kelas bus
        GPIO.output(rgb_pin, GPIO.HIGH)
        time.sleep(1)
        GPIO.output(rgb_pin, GPIO.LOW)
    elif cls == 2: # jika kelas motor
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    # Turn-on LED
                    threading.Thread(target=control_led, args=(cls,)).start()

            # Stream results
            im0 = annotator.result()

            # If video inference, show fps
            if dataset.mode != 'image':
                # currentTIme = time.time()
                fps = 1 / dt[1].dt
                fps_str = "{:.2f}".format(fps)  # Format string dengan 2 angka di belakang koma
                cv2.putText(im0, "FPS: " + fps_str, (20, 70), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255,255,0), 2)
        ...
        LOGGER.info(f"{s}{'' if len(det) else '(no detections), '}{dt[1].dt * 1E3:.1f}ms, {1 / dt[1].dt:.1f}fps")
